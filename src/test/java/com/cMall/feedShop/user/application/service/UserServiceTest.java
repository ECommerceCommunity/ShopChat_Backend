...
class UserServiceTest {
    ...
    // 회원가입 성공 - PENDING 상태에서 재인증 메일 발송
    @Test
    ...
    void signUp_Success_PendingUser_ResendEmail() {
        ...
        given(userRepository.save(any(User.class))).willAnswer(invocation -> invocation.getArgument(0)); // save는 인자를 그대로 반환
        ...
        assertThat(thrown.getMessage()).contains("재인증 메일이 발송되었습니다");
        assertThat(existingPendingUser.getVerificationToken()).isNotNull(); // 새 토큰이 설정되었는지 확인
        assertThat(existingPendingUser.getVerificationTokenExpiry()).isAfter(LocalDateTime.now()); // 만료 시간 업데이트 확인
